...
(function(){
  var ref$, isType, map, filter, foldl, join, flatten, keys, empty, deepcopy, writeFile, build, isNonsense, exprValue, surveyFields, choicesFields, multilingualFields, pruneFalse, fieldnameConversion, typeConversion, choiceTypeConversion, dateTypeConversion, dateKindConversion, locationTypeConversion, metadataTypeConversion, appearanceConversion, newContext, genRange, convertQuestion, genSettings, convertForm, serializeForm, writeForm;
  ...
  convertQuestion = function(question, context, prefix){
    var choiceId, frm, ref$, too, ref1$, key, value, length, range, count, successorRelevance, other, destination, res$, i$, len$, child, this$ = this;
    prefix == null && (prefix = []);
  ...
  genSettings = function(form){
    var formTitle, ref$, ref1$, formId, ref2$;
    formTitle = isNonsense((ref$ = form.metadata) != null ? ref$.htitle : void 8)
      ? form.title
      : (ref1$ = form.metadata) != null ? ref1$.htitle : void 8;
    formId = "build_" + ((ref2$ = form.title) != null ? ref2$.replace(/([^a-z0-9]+)/ig, '-') : void 8) + "_" + Math.floor(new Date().getTime() / 1000);
    return [['form_title', 'form_id'], [formTitle, formId]];
  };
  convertForm = function(form){
    var context, intermediate, res$, i$, ref$, len$, question, languages, languageNames, language, seenFields, choices, warnings, expandLanguages, genSchema, surveySchema, choicesSchema, genLang, surveySimpleFields, genRows, surveyRows, choicesRows, name, entries, entry, warning, this$ = this;
    context = newContext();
    res$ = [];
