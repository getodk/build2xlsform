...
(function(){
  ...
  surveyFields = ['type', 'name', 'label', 'hint', 'required', 'read_only', 'default', 'constraint', 'constraint_message', 'relevant', 'calculation', 'parameters', 'appearance'];
  ...
  genRange = function(range, self){
    ...
    if (!isNonsense(range.min)) {
      result.push(self + " >" + (range.minInclusive === true ? '=' : '') + " " + exprValue(range.min));
    }
    if (!isNonsense(range.max)) {
      result.push(self + " <" + (range.maxInclusive === true ? '=' : '') + " " + exprValue(range.max));
    }
    return result;
  };
  convertQuestion = function(question, context, prefix){
    var choiceId, frm, ref$, too, ref1$, key, value, length, range, count, successorRelevance, other, selectRange, destination, res$, i$, len$, child, this$ = this;
    prefix == null && (prefix = []);
